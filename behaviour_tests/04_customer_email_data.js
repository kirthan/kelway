...
describe('Customer email data from database', function(){
	...
	it('Get Mail summary',function(done){
		restClient.get(
			'/GetMailSummary?customer='+ data.CustomerCode.valid[0],
	...
	it('Get Mailbox list',function(done){
		restClient.get(
			'/ListMailboxes?customer='+ data.MBCustomerCode.valid[0],
			function(err, req, res, obj){
				if(err){ throw(err) }
	...
	it('Get Mail Contact list',function(done){
		restClient.get(
			'/ListContacts?customer='+ data.MCCustomerCode.valid[0],
			function(err, req, res, obj){
				if(err){ throw(err) }
	...
	it('Get list of distribution lists',function(done){
		restClient.get(
			'/ListDlists?customer='+ data.DlistsCustomerCode.valid[0],
			function(err, req, res, obj){
				if(err){ throw(err) }

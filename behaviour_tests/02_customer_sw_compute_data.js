...
describe('Customer ServiceWorks Compute data from database', function(){
	...
	it('Get ServiceWorks Compute summary',function(done){
		restClient.get(
			'/GetSWcomputesummary?customer=' + data[0].CustomerCode,
	...
	it('Get list of virtual machines',function(done){
		restClient.get(
			'/ListVms?customer=' + data[0].CustomerCode,
			function(err, req, res, obj){
				if(err){ throw(err) }
				console.log(obj)
	...
	it('Get virtual disk summary',function(done){
		restClient.get(
			'/ListDiskSummary?customer=' + data[0].CustomerCode,
			function(err, req, res, obj){
				if(err){ throw(err) }
					console.log(obj)
	...
	it('Get virtual disk details',function(done){
		restClient.get(
			'/ListDiskDetail?vmname='+ vmData[0].Name,
			function(err, req, res, obj){
				if(err){ throw(err) }
					console.log(obj)
